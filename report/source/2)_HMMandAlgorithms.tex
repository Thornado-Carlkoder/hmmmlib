...
The evaluation problem of an HMM is: Given an HMM $\lambda$ and a sequence of observations $Y$, what is the probability of observing the sequence $Y$ with respect to $\lambda$?.\todo{slet punktum} This problem can be solved using the Forward and Backward algorithms.
...
The decoding problem of an HMM is: Given an HMM $\lambda$ and a sequence of observations $Y$, what is the most likely sequence of hidden states that produced $Y$ with respect to $\lambda$? \todo{'with respect to' er unødvendig. lambda er allerede nævnt i sætningen.}
...
The Posterior decoding algorithm\cite{Bishop} - given an HMM $\lambda$ and a sequence of observations $Y$ of length $K$ - calculates an array $Z$ of size $K$, which contains the highest a posterior \todo{a posteriori or posterior} probability for each observation in $Y$. 
